...
export function initialQuery(defaults: Selection): QueryState {
  return { ...defaults, tab: 'tokens', sort: 'date', direction: 'desc', page: 1, pageSize: 50 }
}
...
export function reducer(state: State, action: Action): State {
  ...
  switch (action.type) {
    case 'selection':
      return { ...state, query: { ...q, ...action.value, page: 1 } }
    case 'tab':
      return {
        ...state,
        query: {
    ...
    case 'page':
      return { ...state, query: { ...q, page: action.value } }
    case 'pageSize':
      return { ...state, query: { ...q, pageSize: action.value, page: 1 } }
    case 'replace':
    ...
    case 'column':
      return {
        ...state,
        hidden: state.hidden.includes(action.value)
          ? state.hidden.filter((v) => v !== action.value)
          : [...state.hidden, action.value],
      }
    case 'chartMetric':
...
export function readQuery(p: URLSearchParams, defaults: Selection): QueryState {
  ...
  if (
    tab === 'context'
      ...
      : contextSorts.some((v) => v !== 'sessions' && v === sort)
  )
  ...
  return {
    period: periodSchema.catch(defaults.period).parse(p.get('period')),
    bucket: bucketSchema.catch(defaults.bucket).parse(p.get('bucket')),
    from: date('from'),
    to: date('to'),
    ...
    pageSize: positive('pageSize', 50, 200),
  }
}
...
export function DashboardProvider({
  ...
  const [state, dispatch] = useReducer(reducer, {
    query: readQuery(new URLSearchParams(location.search), defaults),
    theme: savedTheme(),
    hidden: [],
    chartMetric: 'total',
  })
  ...
  useEffect(() => {
    const onPop = () =>
      dispatch({
        type: 'replace',
        value: readQuery(new URLSearchParams(location.search), defaults),
      })
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('tokeninsights.theme', state.theme)
    } catch {
