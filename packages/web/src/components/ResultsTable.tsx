...
export function ResultsTable({ data }: { data: Dashboard }) {
  ...
  return (
    <section className="table-panel panel" aria-label={`${labels[query.tab]} details`}>
      <div className="panel-heading">
        <div className="table-title">
          <h2>{labels[query.tab]} breakdown</h2>
          <span className="count-badge">{exactCount(data.rowCount)}</span>
        </div>
        <div className="table-controls">
          <label className="bucket-control">
            ...
            <select
              ...
              value={query.sort}
              onChange={(e) => {
                const sort = sortSchema.safeParse(e.target.value)
          ...
          <Popover.Root>
            <Popover.Trigger className="button subtle">
              <Columns3 size="1em" />
              Columns
            </Popover.Trigger>
            <Popover.Portal>
                  ...
                  .map((s) => (
                    <label key={s.id} className="check-option">
                      <input
                        ...
                        onChange={() => dispatch({ type: 'column', value: s.id })}
                      />
                      {s.label}
                    </label>
                  ))}
              ...
              <tr key={group.id}>
                {group.headers.map((header) => {
                  ...
                  return (
                    ...
                    >
                      <button onClick={() => dispatch({ type: 'sort', value: sort })}>
        ...
        <div className="pagination">
          <label>
            ...
            <select
              ...
              onChange={(e) => dispatch({ type: 'pageSize', value: Number(e.target.value) })}
            >
              {[25, 50, 100, 200].map((n) => (
