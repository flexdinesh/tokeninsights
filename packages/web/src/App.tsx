  ...
  if (bootstrap.error)
    return (
      <main className="startup-state">
        ...
        <button className="button primary" onClick={() => void bootstrap.refetch()}>
          Retry
        </button>
      </main>
    )
  if (!bootstrap.data)
    return (
      <main className="startup-state">
        ...
        <p>Opening TokenInsights…</p>
      </main>
    )
  return (
    <DashboardProvider defaults={bootstrap.data.defaults}>
      <DashboardShell bootstrap={bootstrap.data} />
    </DashboardProvider>
  )
}
    ...
    <div className="app-shell">
      ...
      <main id="dashboard" className="dashboard">
        <section className="page-heading">
          <div>
            <div className="eyebrow page-kicker">YOUR LOCAL USAGE, IN FOCUS</div>
            <h1>
        ...
        {(statusQuery.error || sync.error) && (
          <ErrorBanner
            message={statusQuery.error?.message ?? sync.error?.message ?? 'Request failed'}
            onRetry={() => {
              if (sync.error) sync.mutate()
...
function SyncProgress({ status }: { status: SyncStatus }) {
  const message =
    status.phase === 'resetting'
      ? 'Resetting local usage data for compatibility…'
      : status.phase === 'rebuilding'
        ? 'Rebuilding usage from all configured local harnesses…'
