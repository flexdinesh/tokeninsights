...
export const useBootstrap = () =>
  useQuery({
    queryKey: ['bootstrap'],
    queryFn: ({ signal }) => request('/api/bootstrap', bootstrapSchema, signal),
    staleTime: Infinity,
  })
export const useSyncStatus = () =>
  useQuery({
    queryKey: ['status'],
    queryFn: ({ signal }) => request('/api/status', statusSchema, signal),
...
export function useAnalytics(q: QueryState, revision: number, enabled: boolean) {
  return useQuery({
    queryKey: ['dashboard', queryParams(q).toString(), revision],
    queryFn: ({ signal }) => request(`/api/dashboard?${queryParams(q)}`, dashboardSchema, signal),
    enabled,
...
export function useFacets(q: QueryState, revision: number, enabled: boolean, search = '') {
  ...
  return useQuery({
    ...
    queryFn: ({ signal }) => request(`/api/filters?${params}`, facetsSchema, signal),
    enabled,
  })
}
