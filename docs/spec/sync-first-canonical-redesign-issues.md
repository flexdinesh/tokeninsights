...
| 7. Fixture Conformance Suite | Complete | Language-neutral source fixtures and expected raw, observation, canonical, and diagnostic outputs are implemented. |
...
- [x] Fixtures exist for OpenCode, Pi, and Codex sources.
  - Current state: fixtures live under `packages/cli/internal/pipeline/testdata/conformance`.
- [x] Fixtures define expected raw facts.
  - Current state: expected raw token usage is stored as JSON fixture output.
- [x] Fixtures define expected raw observations.
  - Current state: expected raw observations are stored as JSON fixture output, with repeat-sync behavior still covered by pipeline tests.
- [x] Fixtures define expected canonical facts.
  - Current state: expected canonical token usage is stored as JSON fixture output.
- [x] Fixtures define expected diagnostics.
  - Current state: expected normalization diagnostics are stored as JSON fixture output.
...
- [x] Fixture format is language-neutral enough for future TypeScript checkpoint plugin conformance.
  - Current state: source fixtures use JSONL and expected outputs use JSON.
